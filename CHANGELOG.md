...
### 版本信息
- 官方版本号从 `7.5.0` 递增到 `7.5.1`，`versionCode` 从 `140` 递增到 `141`。
- 经核对，官方 `7.5.1` tag 与 `7.5.0` 指向同一源码提交，因此这一版是版本线与 release notes 同步更新，不包含新的源码提交。

### 发布说明同步
- 官方 release notes 重点强调了主题与搜索页细节、播放器恢复稳定性、竖屏推荐流并发问题、封面与首帧衔接，以及弹幕设置面板布局与文案优化。
- 由于源码提交未变化，这些行为层调整已经包含在此前同步到本仓库的 `7.5.0` 代码基线中，本次主要是跟进官方版本号和发布说明。
