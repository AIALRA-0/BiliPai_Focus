## v0.2.3-alpha.2.focus.1 (2026-09-24)

BiliPai Focus 稳定更新渠道版本，基于上游 BiliPai `0.2.3-alpha.2`（上游基础仍为 alpha）；`versionCode 389`。

[同步] 纳入上游 `6d5496b44..f259fd3fa` 的 6 个后续提交：显示模式、横屏侧栏默认标签、聊天输入避让、大屏首页刷新、移除全局触摸刷新率控制，以及停止个人页离屏骨架动画。

[调整] 首页顶部标签显隐和顺序统一使用上游设置；旧用户升级时按两套旧配置的实际可见结果迁移，并保留原顺序。设置首页新增可搜索的 Focus 专属入口，新用户使用明示的 Focus 默认布局。

[修复] 顶部标签启用后仍被旧 Focus 开关过滤的问题；发布证明缺失时中止发布，避免无证明的资产进入更新渠道。

[发布] Focus Release 保持稳定更新渠道，构建元数据使用单调递增的 `versionCode`，并注明上游 alpha 基础。

---

## v9.1.1-focus.5 (2026-09-23)

BiliPai Focus 基于 BiliPai `v0.2.3-alpha.2`（上游 `versionCode 387`）同步；本版本 `versionCode 388`。

[添加] 吸收分叉以来的订阅与文章、消息与聊天、视频详情和播放器、动态、界面与平台兼容性改进。

[保留] Focus 首页与内容过滤默认行为、关注分组与排序、动态筛选、搜索分区、相关视频控制、历史清理和独立更新链路。

[修复] 融合推荐流请求 Cookie 策略与刷新率回落，减少启动、评论、播放和缓存路径的同步设置读取，保持旧版用户数据升级兼容。

[修复] 隐藏关注分组在动态侧栏仍显示、平板详情关闭相关推荐后仍保留标签、账号切换期间关注事件可能影响新账号，以及 HTTP 412 验证失败提示不清楚。
