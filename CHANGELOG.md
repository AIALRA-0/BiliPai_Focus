...
> 
> - 最新 Focus 发布：`v7.1.2-focus.2`
> - Focus 变更日志：[FOCUS_CHANGLOG.md](FOCUS_CHANGLOG.md)
> - Focus Release Notes：[docs/releases/focus-7.1.2-focus.2.md](docs/releases/focus-7.1.2-focus.2.md)
> - 下方正文仍对应上游官方版本演进
...
### 版本信息
- 版本号从 `7.1.0` 升级到 `7.1.1`，`versionCode` 升级到 `124`。
- 本次为“应用语言切换收口 + 首页多语言文案接通 + 沉浸式安全区修正”的稳定性更新。

### 语言与本地化
- 新增应用语言设置，支持在应用内切换语言，并修复切换后重启流程不稳定、配置恢复不完整的问题。
- 首页分类、设置入口与相关 UI 文案已接入新的多语言资源，英文与繁中字符串同步补齐。
- `MainActivity`、应用启动流程与设置状态联动已补强，确保语言切换后主题和界面状态一起回到正确配置。

### 外观与沉浸式布局
- 外观设置继续收口，深色模式与暗色风格拆分得更清楚，相关设置页和策略测试同步补齐。
- 首页顶部、搜索区和标签栏的沉浸式安全区处理进一步对齐，减少某些系统上状态栏与顶栏之间出现异常缝隙的情况。
- 新增 `BottomSafeAreaPolicy` 及其测试，进一步统一底部安全区和沉浸式布局的约束。

### 稳定性与页面修复
- 首页顶部分类策略、布局几何与本地化映射测试继续补强，避免语言切换后标签逻辑漂移。
- UP 空间动态页的加载策略继续修复，减少出现无限转圈或长时间停滞的问题。
- 多个首页、设置页、视频页与系统栏相关测试同步更新，以覆盖本次布局和语言收口。
