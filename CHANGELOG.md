...
> 
> - 最新 Focus 发布：`v7.1.4-focus.9`
> - Focus 变更日志：[FOCUS_CHANGLOG.md](FOCUS_CHANGLOG.md)
> - Focus Release Notes：[docs/releases/focus-7.1.4-focus.9.md](docs/releases/focus-7.1.4-focus.9.md)
> - 下方正文仍对应上游官方版本演进
## v7.1.4 (2026-03-24)

### 版本信息
- 版本号从 `7.1.3` 升级到 `7.1.4`，`versionCode` 升级到 `127`。
- 本次为“竖屏视频封面/首帧衔接 + 竖屏进度拖动稳定性修复”的播放器维护更新。

### 竖屏视频封面与起播衔接
- 当关闭“点击视频直接播放”并停留在手动起播封面态时，竖屏视频不再被错误套用成 `16:9` 卡片比例。
- 封面到播放器首帧的比例交接继续收口，竖屏视频在起播后不会再因为首帧尺寸判断抖动而被误当成横屏。

### 竖屏进度拖动稳定性
- 竖屏播放器的进度条拖动现在会更稳定地跟随用户 seek 目标位置，不再先回弹到旧进度再跳回新位置。
- 封面态、手动起播态和实际播放态之间的 seek/进度同步继续统一，减少进度显示与真实播放位置不同步的问题。
