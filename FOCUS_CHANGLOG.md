...
## v9.1.1 Focus / focus.5 (2026-09-23)

- 基于上游主线 `6d5496b44`（`0.2.3-alpha.2`）融合自 9.1.1 以来的功能、修复与架构更新，Android `versionCode` 提升至 `388`，保持 Focus 包名和既有发布签名
- 将 Focus 首页标签、关注分组过滤与排序、搜索区段显隐、相关推荐显隐、历史清空入口迁入新版首页、动态、详情和设置结构
- 首页关注流接入上游结构化分页及更新基线，继续补齐过滤后可见批次，并保护快速更改分组配置时的最终呈现状态
- 增加 Room v4→v6 非破坏性迁移，保留旧版搜索历史和屏蔽 UP 数据
- 更新检查继续读取 Focus GitHub Releases，吸收新版 APK 资产与摘要校验；正式构建增加签名连续性门禁
- 吸收上游合并首页 App 半边的 Cookie 隔离和播放器空闲刷新率回落，并修复冷启动 buvid 身份兜底与设置读取问题
- 发布版本名继续递增 Focus 后缀，使旧版 `9.1.1-focus.4` 的应用内更新能够识别新版
- 修复隐藏关注分组在动态侧栏布局仍显示成员内容的问题；平板详情页关闭相关推荐时同步隐藏标签和关联面板
- 关注/取消关注事件现在携带发起账号身份，首页、动态和空间页仅在当前登录账号一致时应用，避免切换账号期间污染关注状态
- 减少启动、评论、播放与缓存路径的同步磁盘读取，并改善 B 站 HTTP 412 验证失败时的提示

## v9.1.1 Focus / focus.1 (2026-06-06)

- Focus 版本线从 `9.0.5-focus.5` 升级到 `9.1.1-focus.1`，`versionCode` 升级到 `223`。
- 完整合并上游 `9.0.6`、`9.0.7`、`9.1.0`、`9.1.1`，吸收动态评论分页、Android 15 下载前台服务、个人空间配色、视频返回、底栏视觉和播放体验等稳定性修复。
- 修复首页推荐手动刷新卡在旧分页尾部的问题：下拉刷新现在从 fresh feed 重新请求，跳过启动预加载缓存，并在成功后重置推荐分页游标。
- 保留 Focus 包名、Focus 更新源、Focus README、搜索显示开关、关注分组过滤、布尔字段兼容和过滤后补页策略。

## v9.0.5 Focus / focus.5 (2026-06-04)

- Focus 版本线从 `9.0.5-focus.4` 升级到 `9.0.5-focus.5`，`versionCode` 升级到 `222`。
- 搜索页三个 Focus 设置项改为正向“显示”语义：`显示大家都在搜`、`显示搜索发现`、`显示搜索历史`。开关打开就是显示对应完整模块，关闭就是完整隐藏，对齐其它 Focus 设置项。
- 补充设置页结构测试，防止后续再次把这三个开关写成“隐藏”或 `!settings.show...` 的反向逻辑。

## v9.0.5 Focus / focus.4 (2026-06-04)

- Focus 版本线从 `9.0.5-focus.3` 升级到 `9.0.5-focus.4`，`versionCode` 升级到 `221`。
- 修复首页冷启动报 `Failed to parse literal '1' as boolean value at path ...` 的根因：动态、空间动态、热门、收藏、导航等首屏链路布尔字段统一兼容数字与字符串布尔值。
- 首页推荐过滤后会继续补足首屏数量，过滤空页不再直接终止分页；冷启动空白会做有限自动重试，减少必须手动刷新一次才有内容的问题。
- 动态页刷新后改用新 offset 继续分页，冷启动空白同样有限重试；继续保留动态页顶部关注分组过滤设置入口。
- 搜索页 Focus 的“隐藏大家都在搜 / 隐藏搜索发现 / 隐藏搜索历史”继续保持完整区块隐藏，并补充结构测试防止只隐藏内容留下标题。

## v8.0.0 Alpha3 Focus / focus.1 (2026-04-25)

### 版本信息
- Focus 版本线从 `7.9.6-focus.1` 升级到 `8.0.0-Alpha3-focus.1`，`versionCode` 升级到 `165`。
- 这次按主线直接并入上游 `7.9.7 / 7.9.8 / 7.9.9 / 8.0.0-Alpha1 / Alpha2 / Alpha3`，并继续保留 Focus 的共存包名、Focus-only 更新源、首页 FOLLOW 分组/排序/自动同步和首用声明入口。

### 上游同步
- 完整吸收视频页“正在看”、搜索发现推荐、首页与列表观看进度、直播横屏聊天回退、评论保存图片二维码、图片预览文字显隐、合集订阅真实接口、下载重试与 403 刷新、播放器拖拽/seek/长按倍速和小窗滑进等这段时间的主线更新。
- 播放器、搜索、动态、空间、直播、下载和设置页均切到新的 Alpha3 上游实现基础，避免继续停留在 `7.9.x` 的旧链路。

### Focus 稳定性收口
- 修复搜索页进入或输入时的焦点/历史/建议链路脆弱点：自动聚焦改成更保守的一次性请求，建议映射再加一层容错，降低搜索框输入闪退概率。
- 修复拖动进度条后 UI 进度偶发冻结的问题：seek 会话现在增加超时脱困，不会因为底层播放器在少数设备上长时间停留在 pending 状态就把进度条永久卡住。
- 修复应用内更新对 `8.0.0-Alpha3-focus.1` 这类预发布 Focus 版本的比较问题；现在 Alpha/RC/focus 组合版本能正确识别高低版本。
- 继续保留并完成当前 Focus 的分页稳定性补丁：搜索发现独立开关、共享分页策略，以及动态默认多关注者页的 `sourceHasMore / visibleHasMore / continuationAllowed` 状态拆分。

### 验证
- 已通过分页、搜索、更新检查与 seek 会话相关的定向单测。
- 已通过 `:app:compileDebugKotlin`、`:app:testDebugUnitTest`、`:app:assembleRelease`。

## v7.9.6 Focus / focus.1 (2026-04-21)

### 版本信息
- Focus 版本线从 `7.5.1-focus.3` 升级到 `7.9.6-focus.1`，`versionCode` 升级到 `164`。
- 这次按主线直接并入上游 `7.5.2 -> 7.9.6` 的正式更新，再把 Focus 的共存包名、Focus-only 更新源、首页 FOLLOW 分组/排序/自动同步链路回贴到新基线。

### 上游同步
- 完整吸收这段时间播放器、直播、弹幕、评论、搜索、空间、下载、设置和首页导航相关的正式更新，包括进度条/seek、评论 gRPC、动态页重写、空间聚合首屏、缓存清理、高清封面、省流量开关和 Android 16 下载兼容等主线能力。
- 播放器、首页、动态、空间和设置页均切到新的上游实现基础，避免继续停留在 `7.5.x` 的旧链路。

### Focus 稳定性收口
- 动态默认多关注者页的默认补页阈值提高到与首页 Follow 接近的可滚动水平；当过滤后可见动态偏少时，会继续主动补页，不再轻易停在几条内容后就误判到底。
- 动态页补齐了更明确的源分页状态，默认多关注者页和单个 UP 页面不再互相阻塞。
- 应用内更新检查增加“BuildConfig 仓库坐标 + Focus 正式仓库 fallback”的双保险；即使本地手工出包把仓库坐标带错，新包仍会回退到 Focus 正式发布仓库继续检查更新。
- Focus 版本 README、release notes 和应用内展示版本已统一切到 `7.9.6-focus.1`。
...
### 版本信息
- 继续跟进上游 `BiliPai v7.5.1` 版本线，当前推荐对外版本更新为 `7.5.1-focus.2`。
- 为保证已发布 `v7.5.1-focus.1 / 161` 用户可直接升级，`versionCode` 继续递增到 `162`。
- 官方 `7.5.1` tag 与 `7.5.0` 仍指向同一源码提交，因此这一版继续不重复 merge upstream 代码，重点是修正播放器 Home 真正回首页的行为并完成补丁发布。

### 播放器 Home 返回主页修复
- 播放器左上角房子按钮现在会强制回到首页根页面，不再只是退回当前页、上一条视频，或被当前栈状态吞掉。
- 播放器专用 `home` 导航改成独立的顶层回首页逻辑，不再复用通用页面跳转中的“已有目标就恢复/弹栈”判断。
- 竖屏全屏场景也一并修正，不会再先退回当前详情页再尝试跳首页。

### Focus 同步与审计
- Focus 版本线、README、release notes 和应用内更新展示版本已统一切到 `7.5.1-focus.2`。
- 当前 `bli` 仓库工作树仅包含这次补丁发版所需改动，未发现跨项目残留。
...
### 版本信息
- 基于上游 `BiliPai v7.5.0` 建立新的 Focus 发布线，当前推荐对外版本为 `7.5.0-focus.1`。
- 为保证已发布 `v7.3.0-focus.2 / 159` 用户可直接升级，`versionCode` 继续递增到 `160`。
- 这一版的策略是完整并入上游 `7.3.1 -> 7.5.0` 的正式更新，再把 Focus 的共存包名、Focus-only 更新源、首页 FOLLOW 定制、空间兼容补丁和竖屏恢复补丁重新贴回新主干。

### 上游 7.3.1 -> 7.5.0 全量同步
- 并入播放器画质模式、首帧与封面衔接、seek 恢复、诊断信息、评论与播放细节修复，继续跟随上游播放器稳定化主线。
- 并入直播前后台恢复、多源重绑、弹幕配置/同步/过滤/行高、缓存清理选择器、合集排序、构建来源验证与动态页回到顶部等后续正式发布能力。
- 并入 `7.5.0` 的空间页聚合首屏加载与界面重构，首屏会优先吃聚合种子，再继续补齐投稿第一页和非首屏必要数据。

### Focus 兼容与稳定化收口
- 搜索结果进入 `UP` 空间时，`SearchUpItem` 与空间投稿列表关键字段继续保留柔性数值解析，兼容接口把数字写成字符串的返回。
- 当空间页命中“默认视频子页 + 用户信息正常 + 聚合或第一页结果可疑空”的场景时，仍会执行一次 `WBI key` 刷新后的单次重试，避免把异常误判成真正的“暂无视频”。
- 空间页空态改为显式区分“仍在加载”“真实空列表”“请求失败”，不再把首屏补页中的中间状态提前渲染成空态。
- 竖屏全屏在应用内跳转、外部页面切换、系统后台返回时，会先清理 inline collapse offset、viewport transform 和临时宽高覆盖，再恢复 shared player 和播放同步，减少部分系统上视频被压扁、黑边或长宽比不一致的情况。

### Focus 定制保留
- `applicationId` 继续保持 `com.android.purebilibili.focus`，与官方版可共存安装。
- 应用内更新继续只识别 Focus 仓库的正式 `release` 资产，并与上游新增的构建来源验证共存，不接官方更新推送。
- 首页 FOLLOW 的过滤、分组、排序、自动同步、首批/分页展示与首用声明入口继续保留在 `7.5.0` 基线之上。
...
### 版本信息
- 基于上游 `BiliPai v7.2.2` 继续维护 Focus 发布线，当前推荐对外版本为 `7.2.2-focus.1`。
- 为保证已发布 `v7.2.0-focus.1 / 155` 用户可直接升级，`versionCode` 继续递增到 `156`。
- 这一版以 `7.2.2` 为基线，保留 Focus 的共存包名、Focus-only 更新源、首页 FOLLOW 定制、首用声明与分组过滤能力。

### 上游 7.2.1 / 7.2.2 同步
- 并入上游消息中心补全：`@我`、回复我的、收到的赞、系统通知等提醒流，以及消息预览解析和对应导航链路。
- 并入上游聊天页、消息收件箱、个人主页、空间页和底部栏相关收口，减少 Focus 后续在社交和跳转链路上的分叉。
- 并入上游播放器、视频详情和评论区更新，包括 seek 预览气泡、小窗播放、竖屏页、全屏覆盖层、评论样式和相关策略测试。

### Focus 定制保留
- `applicationId` 继续保持 `com.android.purebilibili.focus`，可与官方版共存安装。
- 应用内更新继续只跟踪 Focus 仓库的正式 `release` 资产，不接官方更新推送。
- 首页 FOLLOW 的过滤、首批 16 条、随机/时间/聚类排序、刷新稳定化和 Focus 设置入口继续保留在 `7.2.2` 基线上。

### 验证
- 已通过 `:app:testDebugUnitTest`。
- 已通过 `:app:assembleRelease`。

## v7.2.0 Focus / focus.1 (2026-03-25)

### 版本信息
- 基于上游 `BiliPai v7.2.0` 建立新的 Focus 发布线，当前推荐对外版本为 `7.2.0-focus.1`。
- 为保证已发布 `v7.1.4-focus.9 / 154` 用户可直接升级，`versionCode` 继续递增到 `155`。
- 这一版以 `7.2.0` 为基线，保留 Focus 的共存包名、Focus-only 更新源、首页 FOLLOW 定制、首用声明与分组过滤能力。

### 上游 7.2.0 同步
- 并入上游本轮首页卡片、动态卡片、图片预览与列表页相关更新。
- 并入上游新增的专栏数据模型、专栏详情页面、专栏图片预览/共享转场链路，以及历史/搜索到专栏的导航策略。
- 搜索、历史、WebView、视频详情与若干播放器覆盖层逻辑同步到 `7.2.0` 基线，减少后续 Focus 再跟进时的分叉成本。

### Focus 定制保留
- `applicationId` 继续保持 `com.android.purebilibili.focus`，可与官方版共存安装。
- 应用内更新继续只跟踪 Focus 仓库的正式 `release` 资产，不接官方更新推送。
- 首页 FOLLOW 的过滤、首批 16 条、随机/时间/聚类排序、随机排序纯随机策略和刷新稳定化修复继续保留。
