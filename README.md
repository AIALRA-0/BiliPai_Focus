...
| **视频笔记** | 支持私有笔记、AI 总结草稿、富文本编辑、时间戳、Markdown 互通与系统分享 |
...
| **插件系统** | 内置空降助手、去广告、弹幕增强、夜间护眼、今日推荐单、CDN 区域、首访推荐与投屏插件，可扩展架构 |
...
| **🆕 CDN 区域** | 默认关闭；普通视频播放优先同区域 CDN 候选，同时保留原始 URL 兜底 |
| **🆕 首访推荐** | 默认关闭；仅移除 Web 首页推荐接口 Cookie，使推荐流更接近公开游客推荐 |
| **投屏插件** | 内置 DLNA 与 Google Cast 路由，支持投屏发现与播放链路扩展 |
| **插件中心** | 统一管理所有插件，支持独立配置 |
| **🆕 外部插件** | 支持通过 URL 动态加载 JSON 规则插件 |

`首访推荐` 改编自 wangdaodao 的 [TabulaBili](https://github.com/wangdaodaodao/TabulaBili) 与 tjsky 的 [TabulaBili-Plus](https://github.com/tjsky/TabulaBili)。
...
1. 在 [Releases](https://github.com/AIALRA-0/BiliPai_Focus/releases) 页面下载最新 APK
...
- [x] 插件系统核心架构
- [x] 内置插件 (空降助手 / 去广告 / 弹幕增强 / 夜间护眼 / 今日推荐单 / CDN 区域 / 首访推荐 / DLNA / Google Cast)
