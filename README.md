...
<p align="center">
  <sub>最后更新：2026-04-08 · 上游基线 v7.5.1 · 当前 Focus 发布 v7.5.1-focus.1</sub>
</p>
...
| 类别 | 入口 |
| --- | --- |
| 开始使用 | [官方 Releases](https://github.com/jay3-yy/BiliPai/releases) · Focus Releases（当前仓库 Releases 页面） · [Focus Release Notes](docs/releases/focus-7.5.1-focus.1.md) · [更新日志](CHANGELOG.md) · [Focus 变更日志](FOCUS_CHANGLOG.md) |
...
| Focus 定制版 | 想保留原生能力，同时获得更克制的默认入口、关注过滤和专注模式开关 | 当前 Focus 仓库 · 当前仓库 Releases 页面 · [Release Notes](docs/releases/focus-7.5.1-focus.1.md) · [Focus 变更日志](FOCUS_CHANGLOG.md) |

### 当前 Focus 发布

| 项目 | 内容 |
| --- | --- |
| Focus 版本 | `7.5.1-focus.1` |
| 上游基线 | `7.5.1` |
| 发布标签 | `v7.5.1-focus.1` |
| Release Notes | [docs/releases/focus-7.5.1-focus.1.md](docs/releases/focus-7.5.1-focus.1.md) |
| 主要收口 | 跟进官方 `7.5.1` 版本线（官方 tag 与 `7.5.0` 同 commit），并修复播放器左上角 Home 按钮始终直达首页，同时保留 Focus 的共存包名、Focus-only 更新源、FOLLOW 定制和竖屏长宽比恢复补丁 |
| 对外 APK | `BliPai-Focus-release-7.5.1-focus.1.apk` |
...
- 首页“关注”在开启 Focus 关注过滤后，会优先按可见 UP 并发抓取用户动态；主路径继续保持最多 `8` 路并发，同时把展示节奏收口为每组 `16` 条，刷新整页重排、下滑逐组追加。
...
- 当前测试设备仅覆盖 `真我 Neo 7` 与 `联想 Y700 2023`；如果你在其他设备上遇到兼容性或行为问题，请到当前仓库的 `Issues` 页面反馈。
...
| 新增 | `docs/releases/focus-7.5.1-focus.1.md` | Focus `v7.5.1-focus.1` 中文发布说明 |
| 新增 | `docs/releases/focus-7.5.1-focus.1-en.md` | Focus `v7.5.1-focus.1` 英文发布说明 |
...
<p align="left">
  ...
  <a href="docs/releases/focus-7.5.1-focus.1.md">
...
| Focus 定制版 | 在上游基础上补充 Focus 开关、关注分组过滤和更克制的默认入口 | 当前仓库 Releases 页面 |
...
2. 如果你想直接使用这份定制版，请在当前仓库的 Releases 页面下载 APK
...
> [!TIP]
> 路线图最后同步于 2026-03-30（v7.3.0）。功能以最新 Release、`CHANGELOG.md` 与主分支代码为准。
...
### 最近更新 (v7.5.1 / v7.5.1-focus.1 · 2026-04-08)

- 🔄 **跟进官方 `7.5.1` 版本线**：已核对官方 `7.5.1` tag 与 `7.5.0` 指向同一源码提交，因此这次 Focus 同步不重复 merge upstream 代码，只对齐版本线、文档与发布渠道。
- 🏠 **播放器 Home 按钮直达首页**：播放页左上角房子按钮不再复用返回逻辑，普通视频、竖屏全屏和平板布局都会统一直接回首页，不再回上一条视频或偶发失效。
- 📱 **现有稳定性修复继续保留**：空间页聚合首屏、可疑空结果单次重试、竖屏切换长宽比恢复、FOLLOW 定制和 Focus-only 更新链路继续保持在 `7.5.1` 版本线上。
- 🔍 **仓库已复核无跨项目残留**：当前 `bli` 工作树与跟踪文件中未发现意外混入的其他项目文件或关键词残留。
...
Star / fork / issue 等公开入口请以当前仓库页面为准。
