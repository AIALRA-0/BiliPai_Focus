...
class LiveCategorySegmentedControlStructureTest {
    ...
    fun `all tags parent row uses native preset chip row`() {
        ...
        assertTrue(source.contains("AppThemeAdaptiveTabRow("))
        assertTrue(
            Regex("""val effectiveHorizontalPadding = if \(uiStyle == AppUiStyle.MATERIAL3\) \{\s*AppSpacingTokens.None\s*\} else \{\s*horizontalPadding""")
    ...
    fun `live send sheet uses danmaku permission and lifecycle controls heartbeat`() {
        ...
        assertTrue(sheetSource.contains("const val SheetMaxWidthDp = 640f"))
        assertTrue(sheetSource.contains(".widthIn(max = LiveSendDanmakuSheetLayoutSpec.SheetMaxWidthDp.dp)"))
